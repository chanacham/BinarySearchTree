...
public class BinarySearchTree<AnyType extends Comparable<? super AnyType>> {
    ...
    private static class BinaryNode<AnyType> {
        ...
        int count;                  // number of times the element appears
    ...
    private BinaryNode<AnyType> remove(AnyType x, BinaryNode<AnyType> t) {
        ...
        if (compareResult < 0)
            ...
            //decrement to delete one duplicate
            if (t.count > 1) {
                t.count--;
                return t;
            }
            // ELSE, have to delete the node
    ...
    /**
     * private method to determine the size of the tree by recursively traversing each subtree
     *
     * @param t
     * @return
     */
    private int size(BinaryNode<AnyType> t) {
        int counter = 0;
        if (t != null) {
            counter += size(t.left) + size(t.right) + 1;
        }
        return counter;
    }
